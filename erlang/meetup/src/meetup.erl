...
meetup(Year, Month, DayOfWeek, last) -> calc_last(Year, Month, calendar:last_day_of_the_month(Year, Month), day_of_week_to_daynum(DayOfWeek));
meetup(Year, Month, _DayOfWeek, _) -> {Year, Month, 1}.


calc_teenth(GregorianDays, x, x) -> calendar:gregorian_days_to_date(GregorianDays);
calc_teenth(GregorianDays, DayOfWeek, _DowDate) -> 
